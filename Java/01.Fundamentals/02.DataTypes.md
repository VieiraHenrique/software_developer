...
In the last exercise, you had to find a way to declare a decimal number. You probably found some material about data types in Java.
...
As you can see, the difference here is the amount of space each one of these takes in memory.

`byte` for example can only hold numbers from -128 to 127.

Small hint : `int` is used most of the time, even if handling small numbers.
